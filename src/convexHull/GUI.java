...
public class GUI extends Application {
	...
	private HBox lPane;
	/**
	 * These are arrays of coordinates with c1 and c2 being all coordinates and
	 * hull1 and hull2 being hull coordinates for the respective c1 and c2
	 * coordinate arrays
	 */
	 ...
	 * <p>
	 * Creates a new Alert of type INFORMATION before setting the title of said
	 * alert to titleStr and the content to contentStr. The Alert is then shown.
	 * <p>
	 * 
	 * @param titleStr
	 *            is the string used for the title of the window
	 * @param contentStr
	 *            is the string used for the content of the window
	 */
	 ...
	 * <p>
	 * Fills the canvas with a white rectangle to act as the background then draws a
	 * point on the canvas for all the coordinates passed to this function
	 * <p>
	 * 
	 * @param c
	 *            is an array of type Coordinate
	 */
	 ...
	 * 
	 * @param h
	 *            is an array of type Coordinate - should be hull specific
	 */
	public void drawHull(Coordinate[] h) {
		...
		for (int ct = 1; ct < h.length; ct++) {
			...
			gContext.strokeLine(h[ct].getXPosition(), h[ct].getYPosition(), h[ct - 1].getXPosition(),
					h[ct - 1].getYPosition());
		}
		/* Draws a line between the end of the array and the start */
		gContext.strokeLine(h[h.length - 1].getXPosition(), h[h.length - 1].getYPosition(), h[0].getXPosition(),
				h[0].getYPosition());
	}
	 ...
	 * 
	 * @param p
	 *            is an array of type Coordinate - should be polygon specific
	 */
	public void drawPolygon(Coordinate[] p) {
		...
		for (int ct = 0; ct < p.length - 1; ct++) {
			...
			gContext.strokeLine(p[ct].getXPosition(), p[ct].getYPosition(), p[ct + 1].getXPosition(),
					p[ct + 1].getYPosition());
		}
		/* Draws a line between the first and last points in the array p */
		gContext.strokeLine(p[p.length - 1].getXPosition(), p[p.length - 1].getYPosition(), p[0].getXPosition(),
				p[0].getYPosition());
	}
	 ...
	 * <p>
	 * Creates a string and adds the passed coordinates to that string before
	 ...
	 * <p>
	 * Determines how many hulls are being drawn and then outputs the coordinate
	 * data for the hull(s) to the rPane
	 * <p>
	 * 
	 * @param c
	 *            is an array of type Coordinate
	 ...
	 * <p>
	 * Creates all the Menu functions for the MenuBar and then returns the created
	 * MenuBar
	 * <p>
	 * 
	 * @return menuBar
		...
		mInfo.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent actionEvent) {
				...
				alertWindow("Information",
						"This is the Convex Hull program, please select a coursework task, or input your own file.");
			}
		});
	 ...
	 * 
	 * @param lPane
	 *            is a HBox that will hold the buttons
	 */
